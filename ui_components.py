...
def display_stock_card(stock_row, container):
    ...
    foreign_color = "#00ff8c" if stock_row.get('Foreign Flow', 'Outflow') == 'Inflow' else "#ff4d4d"
    card_html = f"""
    <div class="stock-card">
        ...
        <div style="display:flex;justify-content:space-between; font-size:0.9em;">
            <div><div style="color:#a0a0a0">Volume</div><div><strong style="color:white">{stock_row['Volume']/1e6:.1f}M</strong></div></div>
            <div><div style="color:#a0a0a0">Foreign</div><div style="color:{foreign_color}"><strong>{stock_row.get('Foreign Flow', 'N/A')}</strong></div></div>
        </div>
    </div>"""
...
def display_main_metrics(df):
    ...
    if not df.empty:
        ...
        akumulasi_count = df[df['Final Signal'].str.contains("Akumulasi", case=False, na=False)].shape[0]
        distribusi_count = df[df['Final Signal'].str.contains("Distribusi", case=False, na=False)].shape[0]
        flow_delta = "Naik" if net_foreign_flow > 0 else "Turun"
        col1.metric("Net Foreign Flow", f"Rp {net_foreign_flow/1e9:.2f} M", delta=flow_delta, delta_color="normal" if net_foreign_flow > 0 else "inverse")
        col2.metric("Saham Sinyal Akumulasi", f"{akumulasi_count} saham")
        col3.metric("Saham Sinyal Distribusi", f"{distribusi_count} saham")
    else:
        col1.metric("Net Foreign Flow", "N/A"), col2.metric("Saham Sinyal Akumulasi", "N/A"), col3.metric("Saham Sinyal Distribusi", "N/A")
    with st.expander("❓ Apa arti metrik ini?"):
        st.info("- **Net Foreign Flow**: Total nilai pembelian dikurangi penjualan oleh investor asing.\n- **Saham Sinyal Akumulasi**: Jumlah saham yang terdeteksi memiliki sinyal akumulasi.\n- **Saham Sinyal Distribusi**: Jumlah saham yang terdeteksi memiliki sinyal distribusi.")
